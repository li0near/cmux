...
///
/// Schema reference: `Sources/SessionIndexStore.swift:944-1006` —
/// `extractCodexMetadata`. Codex wraps everything in a `payload` object whose
/// shape varies by `type`. We model only the four shapes the inspector needs.
///
/// Unlike Claude JSONL, Codex lines do **not** carry per-line timestamps. We
/// synthesise monotonic timestamps from line index + the file's mtime so the
/// downstream chunk model can still order chunks. See `CodexSyntheticTimestamps`.
...
/// Convenience accessors over a payload's known fields.
