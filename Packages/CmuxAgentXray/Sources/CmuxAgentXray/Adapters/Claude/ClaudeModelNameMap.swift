...
/// Friendly model-name parser for Anthropic model ids.
///
/// Understands both the new `claude-{family}-{major}-{minor}-{date}`
/// format (e.g. `claude-sonnet-4-5-20250929` → "Sonnet 4.5") and the
/// legacy `claude-3-5-sonnet-20241022` / `claude-3-opus-20240229` shapes.
///
/// Returns nil when the id doesn't match any known shape so the caller
...
enum ClaudeModelNameMap {
    static func friendlyName(for modelId: String) -> String? {
        ...
        guard lowered.hasPrefix("claude") else {
            // Non-claude ids (e.g. opencode/codex) — return as-is so
        ...
        // New format: {family}-{major}-{minor}[-{date}]
        ...
        // Legacy: {major}-{minor?}-{family}-{date}
