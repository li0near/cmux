...
/// Read-only mirror of `~/.cmuxterm/codex-hook-sessions.json`. Same
/// schema as the Claude hook store; only the file name differs.
///
/// `@unchecked Sendable` — mutable state guarded by the private
/// dispatch queue. See `ClaudeHookSessionStore` for the shape; this
/// is a near-mirror.
public final class CodexHookSessionStore: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(
        label: "com.cmux.agentXray.codexHookStore",
        qos: .utility
    )
    ...
    public init(
        path: String = CodexHookSessionStore.defaultPath,
    ...
    public func loadAll() -> [String: AgentHookSessionRecord] {
        ...
        for (sid, value) in rawSessions {
            ...
            // Codex stores `rolloutPath` rather than `transcriptPath` in
            // some versions; accept either for forward-compatibility.
            let transcript = (entry["transcriptPath"] as? String)
                ?? (entry["rolloutPath"] as? String)
