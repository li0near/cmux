...
extension TranscriptRoot {
    ...
    ///    set the abandoned entries on the link's top-level
    ///    ``SynthesizedEntry/subEntries`` field (post-G1.5; was
    ///    `body.sections[.subentries(...)]` pre-G1.5).
    ///
    ...
    ///   `[.synthesized(link)]`. The displaced entries' top-level
    ///   index slots are dropped (and their nested descendants too —
    ///   G4 will extend the index if it needs index-driven mutation
    ///   through archived branches; until then archived entries are
    ///   reachable only via the link's subEntries walk).
