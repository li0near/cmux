...
/// entry. Top-level entries display a wall-clock timestamp; tool sub-entries
...
public enum TimeMarker: Equatable, Sendable {
    ...
    case clock(Date)
    /// Runtime in milliseconds (used by tool sub-entries). Renderer
    /// formats as `"\(ms) ms"`.
    case duration(Int)
    ...
    /// Pre-formatted text rendered at the right edge of an entry header.
