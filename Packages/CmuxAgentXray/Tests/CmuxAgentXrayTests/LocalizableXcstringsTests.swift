...
struct LocalizableXcstringsTests {
    ...
    /// `agentXray.row.agent.label`). Reads the raw JSON since SwiftPM
    ...
    func agentLabelsKindSpecificInXcstrings() throws {
        ...
        #expect(strings["agentXray.row.agent.label.claude"] != nil)
        #expect(strings["agentXray.row.agent.label.codex"] != nil)
        #expect(
            strings["agentXray.row.agent.label"] == nil,
