...
- Q: Where do builder-internal pools live (e.g. G4's `awaitingParent` for parent-later JSONL lines)? **Adapter's `BuildContext`, never `Models/Transcript`.** Two reasons: (a) the pool's value type is adapter-specific — `ClaudeJSONLLine` lives in `Adapters/Claude/Wire/`, so putting `[String: ClaudeJSONLLine]` on `Transcript` would couple `Models/` to Claude wire format and invert the layer dependency; (b) the pool's lifecycle is per-builder-invocation scratch (accumulates during one dispatch pass, drains by end of `transcript()`), not persistent like `Transcript.entries`. **Forward rule for future adapters (Codex, etc.):** *a pool's layer is bounded by (i) its value type's layer and (ii) where its lifecycle ends.* Adapter-wire types + per-invocation lifecycle → adapter's `BuildContext`. Domain-model types + persistent lifecycle → the model. The two pools that landed in Phase G (`awaitingParent`, `pendingPromptQueue`) sit on `BuildContext` for both reasons. Resolved during G6 dogfood (commit `b53b3e776`); the design brief had initially flirted with `Transcript`-side placement, which dogfood and the layering audit corrected.
